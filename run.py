...
if __name__ == "__main__":
    ...
    try:
        tg_response = requests.get(TG_API + "/getUpdates", params={"limit": 100, "timeout": 2}).json()
        latest_update_id = int(tg_response["result"][0]["update_id"])
        while tg_response["result"]:
            tg_response = requests.get(TG_API + "/getUpdates", params={"offset": latest_update_id + 1, "limit": 100, "timeout": 2}).json()
